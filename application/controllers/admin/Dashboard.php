...
class Dashboard extends CI_Controller {
  ...
  public function send_notification($user_token='')
  {
    ...
    $getData = $this->db->where('n.sender',$this->session->userdata('chat_token'))
      ->where('n.receiver',$user_token)
      ->from('notification_table as n')
      // ->join('providers as p ','p.token=n.receiver', 'left')
      // ->join('users as u ','u.token=n.receiver', 'left')
      ->select('n.notification_id,n.message,n.created_at,n.utc_date_time')
      ->get();
    if (!$getData) {
