...
class Home_model extends CI_Model
{
    ...
    public function get_category()
    {
        ...
        $this->db->select('c.id, c.category_name, c.category_image, c.description, c.icon, (SELECT COUNT(s.id) FROM services AS s WHERE s.category=c.id AND s.status=1 ) AS category_count');
