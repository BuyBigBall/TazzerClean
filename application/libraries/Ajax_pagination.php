	...
	function create_links(){
		...
		if  ($this->cur_page != 1){
			...
			$output .= $this->prev_tag_open 
					. $this->getAJAXlink( $i, $this->prev_link , "page_prev")
					. $this->prev_tag_close;
		}
// print_r($this->cur_page." ".$start." ".$end); exit;
		...
		for ($loop = $start -1; $loop <= $end; $loop++){
			...
			if ($i >= 0){
				...
				if ($this->cur_page == $loop){
					$output .= $this->cur_tag_open. $this->getAJAXlink( $n, $loop ) . $this->cur_tag_close; // Current page
				}else{
		...
		if ($this->cur_page < $num_pages){
			$output .= $this->next_tag_open 
				. $this->getAJAXlink( $this->cur_page * $this->per_page , $this->next_link , "page_next")
				. $this->next_tag_close;
		}
	...
	function getAJAXlink($count, $text, $class = null) {
      $pageCount = $count?$count:0;
      if(is_numeric($text)){
        	$class='class="pagination_no page_nos_'.$pageCount.'" ';
	   }else{
	    	$class="class=\"pagination_no $class\"";
	   }
